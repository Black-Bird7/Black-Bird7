...
# Fine-grained personal access token (repo secret: ACCESS_TOKEN) with:
...
USER_NAME = os.environ.get('USER_NAME', 'black-bird7')
...
def member_since(created_at_iso):
    """
    'X years, X months, X days' since the GitHub account was created —
    used for the 'GitHub Member Since' field instead of a birthday, since
    that's a real, API-verifiable fact rather than something invented.
    """
        ...
        if repo_hash == hashlib.sha256(edges[index]['node']['nameWithOwner'].encode('utf-8')).hexdigest():
            try:
                if int(commit_count) != edges[index]['node']['defaultBranchRef']['target']['history']['totalCount']:
                    ...
                    data[index] = repo_hash + ' ' + str(edges[index]['node']['defaultBranchRef']['target']['history']['totalCount']) + \
...
def svg_overwrite(filename, member_since_str, commit_data, star_data, repo_data, contrib_data, follower_data, loc_data):
    tree = etree.parse(filename)
    root = tree.getroot()
    justify_format(root, 'uptime_data', member_since_str, 20)
    justify_format(root, 'commit_data', commit_data, 11)
    justify_format(root, 'star_data', star_data, 11)
    justify_format(root, 'repo_data', repo_data, 11)
...
if __name__ == '__main__':
    ...
    since_data, since_time = perf_counter(member_since, acc_date)
    formatter('member-since calc', since_time)
    ...
    svg_overwrite('dark_mode.svg', since_data, commit_data, star_data, repo_data, contrib_data, follower_data, total_loc[:-1])
    svg_overwrite('light_mode.svg', since_data, commit_data, star_data, repo_data, contrib_data, follower_data, total_loc[:-1])
